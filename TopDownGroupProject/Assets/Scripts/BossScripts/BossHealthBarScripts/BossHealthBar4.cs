...
public class BossHealthBar4 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        bossObject.GetComponent<BossAI4>().bulletLifetime = 0f;
    }
    //UDPATE FUNCTION
    void Update()
    {
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            player.GetComponent<PlayerShoot>().shootDelay = 0.1f;
            barOn = true;
            bossHUD.GetComponent<Canvas>().enabled = true;
            bossObject.GetComponent<BossAI4>().bossActive = true;
            bossObject.GetComponent<BossAI4>().bulletLifetime = 10f;
        }
        else if (collision.gameObject.tag == "Player" && barOn == true)
        {
            barOn = false;
            bossHUD.GetComponent<Canvas>().enabled = false;
            bossObject.GetComponent<BossAI4>().bossActive = false;
            bossObject.GetComponent<BossAI4>().bulletLifetime = 0f;
        }
    }
}
