...
public class BossHealthBar2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        bossObject.GetComponent<BossAI2>().bulletLifetime = 0f;
    }
    //UDPATE FUNCTION
    void Update()
    {
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Player" && barOn == false)
        {
            ...
            bossObject.GetComponent<BossAI2>().bossActive = true;
            bossObject.GetComponent<BossAI2>().bulletLifetime = 10f;
        }
        else if (collision.gameObject.tag == "Player" && barOn == true)
        {
            barOn = false;
            bossHUD.GetComponent<Canvas>().enabled = false;
            bossObject.GetComponent<BossAI2>().bossActive = false;
            bossObject.GetComponent<BossAI2>().bulletLifetime = 0f;
        }
    }
}
